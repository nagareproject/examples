...
# No more hard coded rules : we enhance the ``User`` objects with a list of
# permissions
...
class Authentication(form_auth.Authentication):
    ...
    def _create_user(self, username):
        ...
        if username == 'john':
            return User(username, ('wiki.editor',))
        
        if username == 'admin':
            return User(username, ('wiki.admin', 'wiki.editor'))
        
        return User(username)


class UserWithPermissionsRules(common.Rules):
    # A logged user wants to access an object protected by the 'perm'
    # permission
